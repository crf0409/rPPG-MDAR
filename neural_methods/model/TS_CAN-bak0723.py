...
class SELayer(nn.Module):
    ...
    def forward(self, x):
        ...
        self.fc[0] = nn.Linear(c, c // self.reduction, bias=False).to(device)
        self.fc[2] = nn.Linear(c // self.reduction, c, bias=False).to(device)
...
#         #     # 初始化时动态创建卷积层
#         #     self.fc = nn.Sequential(
#         #         nn.Conv2d(c, c // self.reduction, 1, bias=False).to(x.device),
#         #         nn.ReLU(inplace=True),
#         #         nn.Conv2d(c // self.reduction, c, 1, bias=False).to(x.device)
#         #     )
...
class ChannelAttention(nn.Module):
    ...
    def __init__(self, ratio=16):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Conv2d(128, 128 // ratio, 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(128 // ratio, 128, 1, bias=False)
        )
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x):
       ...
       # 动态修改全连接层的输入输出通道数
       device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
       self.fc[0] = nn.Conv2d(c, c // 16, 1, bias=False).to(device)
       self.fc[2] = nn.Conv2d(c // 16, c, 1, bias=False).to(device)
       avg_out = self.fc(self.avg_pool(x))
       max_out = self.fc(self.max_pool(x))
       out = avg_out + max_out
       out = self.sigmoid(out)
...
class CBAM(nn.Module):
    ...
    def __init__(self, ratio=16, kernel_size=3):
